...
### Added
- **Previews Before Proposals**: while analyzing your request the assistant can now dry-run scene and asset changes — it checks that the target exists, which components and fields it would touch, and reports exactly what would change, without changing anything. Recommendations and plans get noticeably more precise as a result, and the analysis turn still never mutates your project. Every scene/asset tool also accepts `dry_run: true` during plan execution, so the assistant can validate a destructive batch before running it.
- **One Ctrl+Z Per Plan Step**: every scene change a plan step makes — including anything a retry re-applied — now collapses into a single Undo entry named after the step ("AI Familiar: Step 1/3 — …"), so reverting a step is one keystroke. Cancelling a batch of tool calls mid-way no longer leaves a half-open Undo group.
- **Instant Conversation Titles**: a new conversation's tab is titled the moment you send the first prompt (derived from your message), and a few seconds later a tiny background model call refines it into a short summary — no more tabs stuck on "Untitled". If that call fails or you're offline, the derived title simply stays; it runs alongside your actual request and never delays it.
- **Pause A Running Plan**: the Executing Plan bar's pause button now genuinely pauses — execution holds once the current step finishes (the button dims to "pausing…" until it does), and the same button becomes a resume control. A pause even survives the script recompiles that plan steps can trigger. Previously that button cancelled the plan outright; stopping is now only the stop button's job.

### Changed
- **A Calmer, Easier-To-Scan Conversation**: the transcript is redesigned around a timeline. Everything the assistant does — reading files, searching, inspecting your scene — renders as a compact one-line entry on a vertical spine with a status dot (blue while running, green or red when done) and its outcome right on the line ("Found 14 files"), expandable for full details. Runs of investigation work are grouped under a small "Investigating · N tool calls" caption, your messages and the assistant's replies become clean labeled blocks with timestamps, and the only framed cards left are the ones asking for your decision — plan approvals and suggested actions — so they stand out instead of competing with a wall of boxes.

### Fixed
- **Settings Now Persist On Fresh Installs**: a 1.35.0 regression made the plugin skip creating its settings file on any project where one didn't already exist — everything you changed in Settings looked saved but silently reverted to defaults on the next script recompile or editor restart. API keys were likewise kept only in a machine-wide backup instead of their intended per-project store. Both now save properly, including on the very first run in a brand-new project.
- **No More Raw Markdown In Cards**: approving a suggested action no longer shows the step header as raw markdown ("## What I'll do…", stray `**`, table pipes). Step cards now carry a clean one-line title and render the full description as formatted markdown inside the card body — and the same treatment covers every other surface that shows model text: plan summaries, notice/error card headers, the execution rail, plan review, the plan editor's read-only view, todo items, and more.
- **Readable Tool Results**: tool cards no longer dump raw JSON cut off mid-token. Every result gets a human summary line — with natural per-tool phrasing like "Created Root/Props/Crate", "12 passed, 1 failed", or "Downloaded wooden_crate_02 from Poly Haven — 3 files, 2.3 MB" — plus a plain-language Details foldout ("Label: value" lines and bullets, file sizes as KB/MB) instead of a JSON block. Failed calls are marked in red with their error selectable for copy-paste, reopened conversations show their tool results again (history replay previously dropped them), and results saved by older versions are converted to the readable format when displayed.
- **No More Leftover Plan Card After Taking An Action**: clicking **Take action** on a suggested action no longer drops a redundant "Plan · 1 step" card into the conversation — one that appeared mid-execution still offering Approve / Regenerate / Cancel. The suggestion card and the step below it now tell the whole story. **Make a plan instead** still opens the full, editable plan card.
- **Empty "Thinking" Rows Are Gone**: a thinking row now appears only when there is reasoning to read. Blocks the model returns without readable text — encrypted ones, and every block when **Show Thinking** is off on a model whose thinking can't be switched off — no longer leave a row that expands onto an empty panel, in new conversations and reopened ones alike.
- **Tools That Take No Arguments Work Again**: calls like saving the active scene in place came back as "Error: Invalid JSON arguments" whenever the assistant invoked them the documented way — with no arguments at all. Every no-argument tool now runs normally.
- **Lists And The Text Around Them Render Correctly**: when the assistant wrote a list with blank lines between its items, some items were followed by visibly taller gaps than the rest — the list now keeps even spacing throughout. Worse, a sentence placed between two list items (a short note after step 1, say) could vanish from the reply entirely; it now shows up where it was written.
